...
def main():
    ...
    if knn_args.retomaton or knn_args.cluster_dstore:
        knn_wrapper = RetomatonWrapper(dstore_size=knn_args.dstore_size, dstore_dir=knn_args.dstore_dir, 
            ...
            no_pointer=knn_args.no_pointer, min_knns=knn_args.min_knns, max_knns=knn_args.max_knns,members=knn_args.members)
    ...
    def compute_metrics(eval_preds):
        print("call compute_metrics function")
        print("eval_preds is",eval_preds)
        ...
        results={'acc':1}
        #results = accuracy_metric.compute(preds,labels)
        return results
    ...
    def build_qa_prompt(example, fewshot):
        if fewshot:
            few_shot_examples = [
    ...
    def evaluate_dataset(
        ...
        for ex in (tq := tqdm(eval_dataset, desc=f"F1:  0.0%")):
            ...
            losses = []
            prompt = build_qa_prompt(ex, False)
            batch_size = 2
            # for i in range(0, len(answers), batch_size):
            #     batch_answers = answers[i:i+batch_size]
            #     for answer in batch_answers:
            #         losses.append(compute_loss(tokenizer, model, device, prompt, answer, max_length))
            # if losses:
            #     avg_loss = sum(losses) / len(losses)
            #     pred_losses.append(avg_loss)
            if idx == 0:
            ...
            with torch.no_grad():
                if 'gpt' in model.name_or_path.lower():
                    ...
                    # with open("tokens.txt", "a") as file:  # 打开文件以追加模式
                    #     file.write("Answers: ")
            ...
            for answer in answers:
                ...
                if f1 > max_f1:
                    ...
                    best_recall = recall
                print(answer)
            print(max_f1,best_prec, best_recall)
        ...
        # eval_loss = sum(pred_losses)/len(pred_losses)
        F1 = sum(f1_list)/len(f1_list) * 100
        # print(len(f1_list), len(pred_losses))
        print(f"EM: {em:.2f}%")
