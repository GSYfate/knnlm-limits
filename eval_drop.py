...
def main():
    ...
    def f1_score(prediction, ground_truth):
        ...
        if is_number(normalized_ground_truth):
            numbers_in_prediction = re.findall(r'\b\d+\b', normalized_prediction)
            if normalized_ground_truth in numbers_in_prediction:
                return ONE_METRIC
            else:
                return ZERO_METRIC
    ...
    def evaluate_dataset(
        ...
        for ex in (tq := tqdm(eval_dataset, desc=f"F1:  0.0%")):
            answers = ex['answers_spans']['spans']
